...
class Dynamic_Controller():
    ...
    def forward_m(self, s):
        ...
        input_mat = np.array([s])
        feature_norm = torch.from_numpy(self.input_scaler.transform(input_mat)).to('cpu').float()
        target_norm_hat = self.model.forward_m(feature_norm)
        target_hat_mat = self.output_scaler.inverse_transform(target_norm_hat.detach().numpy())
        return [target_hat_mat[0,i] for i in range(target_hat_mat.shape[1])]
    def forward_c(self, s):
        """
        mapping state to action
        :param state s = [q1, q2, qd1, qd2, qdd1, qdd2]
        :return: a = [a1, a2]
        ...
        input_mat = np.array([s])
        feature_norm = torch.from_numpy(self.input_scaler.transform(input_mat)).to('cpu').float()
        target_norm_hat = self.model.forward_c(feature_norm)
        target_hat_mat = self.output_scaler.inverse_transform(target_norm_hat.detach().numpy())
        return [target_hat_mat[0,i] for i in range(target_hat_mat.shape[1])]
    def forward_g(self, s):
        """
        mapping state to action
        :param state s = [q1, q2, qd1, qd2, qdd1, qdd2]
        :return: a = [a1, a2]
        ...
        input_mat = np.array([s])
        feature_norm = torch.from_numpy(self.input_scaler.transform(input_mat)).to('cpu').float()
        target_norm_hat = self.model.forward_g(feature_norm)
        target_hat_mat = self.output_scaler.inverse_transform(target_norm_hat.detach().numpy())
        return [target_hat_mat[0,i] for i in range(target_hat_mat.shape[1])]

    def forward_all(self, s):
        """
        mapping state to action
        :param state s = [q1, q2, qd1, qd2, qdd1, qdd2]
        :return: a = [a1, a2]
        ...
        input_mat = np.array([s])
        feature_norm = torch.from_numpy(self.input_scaler.transform(input_mat)).to('cpu').float()
        m, c, g = self.model.forward_all(feature_norm)
        m_out = self.output_scaler.inverse_transform(m.detach().numpy())
        c_out = self.output_scaler.inverse_transform(c.detach().numpy())
        g_out = self.output_scaler.inverse_transform(g.detach().numpy())
        return [m_out[0,i] for i in range(m_out.shape[1])], [c_out[0,i] for i in range(c_out.shape[1])], [g_out[0,i] for i in range(g_out.shape[1])]
